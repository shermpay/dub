...
std::unique_ptr<semantic::TypeMismatchError> MakeTypeMismatchError(
    Type want, const Expression& expr, Type got) {
  return std::make_unique<semantic::TypeMismatchError>(want, &expr, got);
}

std::unique_ptr<semantic::TypeNameUndefinedError> MakeTypeNameUndefinedError(
    const Symbol& name, const Expression& expr) {
  return std::make_unique<semantic::TypeNameUndefinedError>(name, &expr);
}

namespace {

// MakeType is a visitor that interprets type expressions and constructs `Type` instances.
struct MakeType final {
  ...
  absl::StatusOr<Type> operator()(const Symbol* name) const {
    ...
    if (!type.has_value()) {
      return absl::NotFoundError(absl::StrFormat("type name '%v' is undefined", name));
    }
    return type.value();
  }
  ...
  absl::StatusOr<Type> operator()(const TypeExpr::Construct expr) const {
    ...
    if (!ctor.has_value())
      return absl::NotFoundError(absl::StrFormat("type constructor '%v' is undefined", expr.name()));
...
// It verifies the types of the expressions and returns the type of the expression.
struct Check final {
  Typer& typer;
  ...
  Result<Type> operator()(const compiler::Constant& ce) {
    if (ce.Is<Nil>()) {
      ...
      // This would require deducting the type or using a type assertion of sorts.
    } else if (ce.Is<bool>()) {
      return type::Bool();
  ...
  Result<Type> operator()(const Call& expr) {
    ...
    if (!callee_type.has_value()) {
      return Result<Type>(MakeTypeNameUndefinedError(*callee, the_expr));
    }
    ...
    for (size_t i = 0; i < expr.args().size(); ++i) {
      ...
      if (arg_type.Value() != param_types.types()[i]) {
        return Result<Type>(MakeTypeMismatchError(
    ...
    if (*callee == Symbol::Get("add-i64")) {
      std::cout << "Adding BuiltinInfo to Expression: " << the_expr << std::endl;
      // TODO: This won't work for high-order funcs
      const auto& arg_types = param_types.types();
      if (arg_types.size() == 2) {
        if (type::IsIntegerType(arg_types[0]) && type::IsIntegerType(arg_types[1])) {
          typed_module.AddBuiltinInfo(the_expr,
                                      BuiltinInfo(Builtin::kIntAdd));
        }
      }
    }
  ...
  Result<Type> operator()(const Fn& fn) {
    ...
    if (!type.has_value()) {
      return Result<Type>(MakeTypeNameUndefinedError(*fn.name(), the_expr));
    }
  ...
  Result<Type> operator()(const Symbol* name) {
	...
	if (!type.has_value()) {
      return Result<Type>(MakeTypeNameUndefinedError(*name, the_expr));
	}
	return type.value();
  }
  ...
  Result<Type> operator()(const auto& expr) {
    ...
    std::cerr << "warning: ignoring type of expression => " << expr << std::endl;
    return type::Unit();
  }
};
...
Result<Type> Typer::TypeExpression(const Expression& expr, TypedModule* typed_mod) {
