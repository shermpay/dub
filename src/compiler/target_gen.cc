...
llvm::Expected<std::unique_ptr<TargetGen>> TargetGen::CreateDefault() {
  const auto triple_str = llvm::sys::getDefaultTargetTriple();
  const auto triple = llvm::Triple(triple_str);
  std::string err;
  auto target = llvm::TargetRegistry::lookupTarget(triple, err);
  if (!target) {
    return llvm::createStringError(err);
  }

  auto cpu = "generic";
  auto features = "";
  llvm::TargetOptions opts;
  auto machine = target->createTargetMachine(triple, cpu, features, opts,
                                             llvm::Reloc::PIC_);

  return std::make_unique<TargetGen>(
      triple, std::unique_ptr<llvm::TargetMachine>(machine));
}
...
llvm::Error TargetGen::GenerateModule(llvm::Module *modul,
