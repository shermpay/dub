...
/*
  ...
  Basic idea is to cast underlying memory into structs of more and more concrete
  types.

  "(if foo 1 2)" =Read=> `List` =Parse=> `If`

  As such the data is the same, just how we interpret it. So we don't want to
  copy the data.
...
public:
  ...
  template <typename... Args> static List Make(Args... args) {
    std::vector<T> vals;
    (vals.push_back(args), ...);
  ...
  auto Size() const noexcept { return values_->size() - head_idx_; }
  ...
  const T Head() const { return (*values_)[head_idx_]; }

  const T Second() const { return (*values_)[head_idx_ + 1]; }

  const List<T> Tail() const { return List(this->values_, head_idx_ + 1); }

  auto cbegin() const noexcept { return values_->cbegin() + head_idx_; }
...
struct Form {
  ...
  using Value = std::variant<Nil, bool, std::int64_t, double, std::string,
                             const Symbol *, form::List<Form>, Vector>;
  Value value;
...
public:
  ...
  void Append(const Form &form) { forms_.push_back(form); }

private:
  form::ListContainer &Data() { return forms_; }
