...
// For the default case it's just Reader<void>, but for other cases it's, Reader<SourceInfo>.
class Reader {
 public:
  ...
  absl::StatusOr<std::optional<Form>> ReadForm();
  absl::StatusOr<List> ReadAll();
 ...
 private:
  ...
  absl::StatusOr<Form::Value> ReadLiteralOrSymbol(char first);
  absl::StatusOr<List> ReadList();
  absl::StatusOr<Vector> ReadVector();
